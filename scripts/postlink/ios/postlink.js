...
module.exports = function() {
    ...
    if (parsedInfoPlist.CodePushDeploymentKey) {
        ...
        writePatches();
        return AddPublicKeyToPList(parsedInfoPlist);
    } else {
    ...
    // 4. Add CodePushPublicKey to plist file
    function AddPublicKeyToPList(parsedInfoPlist){
        if (parsedInfoPlist.CodePushPublicKey) {
            console.log(`"CodePushPublicKey" already specified in the plist file.`);
            writePatches();
            return Promise.resolve();
        } else {
            return askForPublicKey('ios')
            .then(function(publicKey){
                parsedInfoPlist.CodePushPublicKey = publicKey;
                plistContents = plist.build(parsedInfoPlist);
                writePatches();
                return Promise.resolve();
            })      
